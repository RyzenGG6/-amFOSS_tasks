...
fn main() {
    ...
    for element in document.select(&_crypto_selector) {
        ...
        let _crypto_price_element = element.select(&_crypto_price_selector).next().expect("couldnt select Crypto price");
        let crypto_price = _crypto_price_element.text().collect::<String>();

        let _crypto_24hourchange_element = element.select(&_crypto_24hourchange_selector).next().expect("couldnt select 24 hour change");
        let crypto_24hourchange = _crypto_24hourchange_element.text().collect::<String>();

        let _crypto_24hourvolume_element = element.select(&_crypto_24hourvolume_selector).next().expect("couldnt select 24 hour volume");
        let crypto_24hourvolume = _crypto_24hourvolume_element.text().collect::<String>();

        let _crypto_marketcap_element = element.select(&_crypto_marketcap_selector).next().expect("couldnt select marketcap");
